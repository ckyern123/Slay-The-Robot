...
func perform_action():
	...
	for action_interceptor_processor in action_interceptor_processors:
		# combat energy
		var energy_amount: int = (action_interceptor_processor.get_shadowed_action_values("energy_amount", 0)) + (Global.player_data.player_insight/8)
		Global.player_data.player_energy = max(0, energy_amount + Global.player_data.player_energy)
		# max energy (permanent)
		var energy_amount_max: int = (action_interceptor_processor.get_shadowed_action_values("energy_amount_max", 0)) + (Global.player_data.player_insight/8)
