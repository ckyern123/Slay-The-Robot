...
func perform_action() -> void:
	...
	for action_interceptor_processor in action_interceptor_processors:
		var card_durability: int = action_interceptor_processor.get_shadowed_action_values("card_durability", -1)
		...
		for card_data in picked_cards:
			if card_durability > -1:
				card_data.subtract_card_durability(card_durability)
