...
const CARD_RARITY_TO_PRICE_RANGE: Dictionary = {
	CardData.CARD_RARITIES.COMMON: [2,4],
	CardData.CARD_RARITIES.UNCOMMON: [3,6],
	CardData.CARD_RARITIES.RARE: [4,8],
}
...
func visit_shop() -> void:
	...
	if not shop_is_visited:
		...
		var generated_cards: Array[CardData] = Random.generate_rarity_weighted_card_draft(rng_shop, ShopData.GENERATED_CARD_COUNT, Random.CARD_DRAFT_TABLE_TYPES.STANDARD, false)
		### Generate Items
		# generates shop cards
